...
commit' = do
        ...
        cur <- cdLines
        {-_ <- addDiff (getGroupedDiff cur (insert "end" ("start":fs)))-}
        _ <- addDiff (getGroupedDiff cur fs)
        gr <- L.use graph
        liftIO $ putStrLn "here1"
        liftIO $ print (getGroupedDiff cur (insert  "post-end" $ insert "end" $ fs))
        return ()


{-view :: a -> Diff a-}
-- TODO: check that recursive calls are getting passed the right node.
-- TODO: This should use takeWhile first or second
...
addDiff' n ds     =
        ...
        in case (f, s) of
           ...
           (Nothing, Nothing) -> addLines [] n (n + 1)   << iter (n + 1)
